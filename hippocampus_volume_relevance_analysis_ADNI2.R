...
Hippocampus_values$HippBoth_HarvOx_MNI <- Hippocampus_values$HippBoth_HarvOx_MNI / 1000
...
save(list = c('hp_lm','GMvol_lm'), file='linear_model_hipp_vol_ADNI2.RData')
...
write.csv(cbind(Hippocampus_values$subject_ID, hipp_vol_res, GMvol_res), file="residualized_vol_ADNI2_n663.csv", row.names=F)
...
print(cor(GMvol_res, Hippocampus_values$sum_relevance_brain, use="pairwise.complete.obs"))
print(cor(hipp_vol_res, Hippocampus_values$sum_relevance_brain, use="pairwise.complete.obs"))
print(cor(hipp_vol_res, GMvol_res))
...
print(cor(hipp_vol_res, Hippocampus_values$sum_relevance_brain_wo_temporal, use="pairwise.complete.obs"))
  correls_cv_brain <- sapply(grep("Sum_relevance_brain_cv", colnames(Hippocampus_values)), function(x) do.call(cor, list(x=Hippocampus_values[,x], y=GMvol_res, use="pairwise.complete.obs")))
...
text(x=2, y=-4.1, labels=paste0("r = ", round(cor_both$estimate, digits=2),
...
# scatter plot of GM vol vs. rel
lm_both <- lm(GMvol_res~Hippocampus_values$sum_relevance_brain) # keeps scale and intercept
cor_both <- cor.test(GMvol_res, Hippocampus_values$sum_relevance_brain) # same as beta of lm(scale(vol_res),scale(act))
plot(jitter(Hippocampus_values$sum_relevance_brain, factor=200), GMvol_res,
     cex = 0.7, pch=pchs[Hippocampus_values$Group], col=mycolors[Hippocampus_values$Group],
     axes = FALSE,
     xlab = "Total relevance of gray matter voxels", ylab = "Bilateral gray matter volume (residuals, ml)",
     main = "Correlation of gray matter volume and CNN relevance scores")
legend(x = "topright", legend=c("Normal", "MCI", "AD"), col=c(1,3,4), pch=c(16,15,17), bty="n")
axis(1, at=NULL, labels=T)
axis(2, at=NULL, labels=T)
abline(a=lm_both$coefficients[1], b=lm_both$coefficients[2], col="red")
text(x=20, y=-150, labels=paste0("r = ", round(cor_both$estimate, digits=2),
                                  ", p ", ifelse(cor_both$p.value<0.001, "< 0.001", paste0("= ", round(cor_both$estimate, digits=2))),
                                  ", R� = ", round(summary(lm_both)$r.squared, digits=2),
                                  ", n = ", cor_both$parameter+2), col="red")
...
axis(2, at=NULL, labels=T)
abline(a=lm_both$coefficients[1], b=lm_both$coefficients[2], col="red")
text(x=20, y=-4.2, labels=paste0("r = ", round(cor_both$estimate, digits=2),
                                  ", p ", ifelse(cor_both$p.value<0.001, "< 0.001", paste0("= ", round(cor_both$estimate, digits=2))),
                                  ", R� = ", round(summary(lm_both)$r.squared, digits=2),
                                  ", n = ", cor_both$parameter+2), col="red")
...
thr <- coords(roc(group!="CN" ~ LRP_hipp, data=dat), x="best", input="threshold", ret=c("thr", "acc", "sens", "spec"), best.method="youden", transpose=T)
print(thr)
...
# group separation by GM relevance scores --> comparable to raw CNN AUC for that specific model/CV-fold
thr <- coords(roc(group!="CN" ~ LRP_brain, data=dat), x="best", input="threshold", ret=c("thr", "acc", "sens", "spec"), best.method="youden", transpose=T)
print(thr)
p3 <- ggplot(dat, aes(x=factor(group, labels=c("CN", "MCI", "AD")), y=LRP_brain)) +
  geom_violin() +
  geom_hline(aes(yintercept=thr[1], color="red")) +
  labs(x="Groups", y="Gray matter relevance scores (log10)") +
  theme(legend.position="none") +
  scale_y_continuous(trans='log10') +
  geom_boxplot(width=0.2, aes(fill=factor(group))) +
  scale_fill_manual(values=mycolors) +
