...
def select_subject_worker():
    ...
    v.update_subject_divs(index_lst[sorted_xs.index(
        v.subject_select.value)])  # called with subj_id; corresponding RID/sid would be m.grps.iloc[m.index_lst[m.sorted_xs.index(v.subject_select.value)], 1]
...
def select_subject_callback(attr, old, new):
    ...
    # In that case firstrun would already be set to False and the v.update_guide_*() calls
